...
class PipelineParallelTransformerModel(BaseFairseqModel):
    def __init__(self, encoder, decoder, balance, devices, chunks, checkpoint):
        ...
        ## 错误:两个rank设备device是一样的
        print(f"rank={torch.distributed.get_rank()} self.devices={self.devices} line:101 pipeline_parallel_transformer\model.py")

        ## 都在CPU上
        # i=0
        # for lay in module_list:
        #     for a,b in lay.state_dict().items():
        #         if i %10 ==0:
        #             print(f"line:101 model.py rank={torch.distributed.get_rank()}",a, b.device)
        #         i+=1
    ...
    def forward(self, src_tokens, src_lengths, prev_output_tokens):
        ...
        # print(f"model.py line 143 self.devices={self.devices}")
    ...
    def build_model(cls, args, task):
        ...
        print(f"model.py line 328 rank={torch.distributed.get_rank()} args.pipeline_balance={args.pipeline_devices}")
        # for k in list(vars(args).keys()):
        #     print(f' rank={torch.distributed.get_rank()} {k}: {vars(args)[k]}')
