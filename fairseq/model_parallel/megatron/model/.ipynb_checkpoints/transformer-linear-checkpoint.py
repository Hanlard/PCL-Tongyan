...
class ParallelSelfAttention(MegatronModule):
    ...
    def __init__(self, attention_mask_func, init_method,
        ...
        self.norm_factor = math.sqrt(args.seq_length)
        if self.apply_query_key_layer_scaling:
    ...
    def forward(self, hidden_states, attention_mask, layer_past=None,
        ...
        key_layer = key_layer.unsqueeze(dim=-1)
        key_layer = self.kernel_fc(key_layer) + 1.
        # [b, np, sq, 1, hn]
        value_layer = value_layer.unsqueeze(dim=-2)
        
        #[b, np, sq, hn, hn] 在 sq维累加
        KV = torch.matmul(key_layer,value_layer)
        KV = torch.cumsum(KV,dim=2)*1.0/self.norm_factor
        with mpu.get_cuda_rng_tracker().fork():
            KV = self.attention_dropout(KV)

        del value_layer
        # [b, np, sq, 1, hn]
        query_layer = query_layer.unsqueeze(dim=-2)
        query_layer = self.kernel_fc(query_layer) + 1.
        # [b, np, sq, 1, 1] 在 sq维累加
        sigmaQK = torch.matmul(query_layer,key_layer)
        sigmaQK = torch.cumsum(sigmaQK,dim=2)
        del key_layer
        # [b, np, sq, 1, hn]
        context_layer = torch.matmul(query_layer,KV)/sigmaQK
        del query_layer
